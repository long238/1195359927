...
void CCMainWindow::initControl()
{
	...
	appupLayout->addStretch();//设置拉伸
	appupLayout->setSpacing(2);//设置部件间的空隙
...
QString CCMainWindow::getHeadPicturePath()
{
	...
	{
		QSqlQuery queryPixture(QString("SELECT picture FROM tab_employees WHERE employeeID = %1").arg(gLoginEmployeeID));
		queryPixture.exec();
		queryPixture.next();

		strPicturePath = queryPixture.value(0).toString();
	...
	else//账号登录
	{
		...
		QSqlQuery queryPixture(QString("SELECT picture FROM tab_employees WHERE employeeID = %1").arg(employeeID));
		queryPixture.exec();
		queryPixture.next();
...
void CCMainWindow::initContactTree()
{
	...
	RootContatltem *pItemName = new RootContatltem(true, ui.treeWidget);
...
void CCMainWindow::setUserName(const QString & userName)
{
	...
	//文本过长则进行省略...
	//fontMetrics()返回字体体积大小，及QFontMetrics对象
...
QWidget * CCMainWindow::addOtherAppExtension(const QString & appPath, const QString & appName)
{
	...
	painter.drawPixmap((btn->width() - appPixmap.width()) / 2, (btn->height() - appPixmap.height()) / 2,
		appPixmap);
	btn->setIcon(pixmap);
...
void CCMainWindow::addCompanyDeps(QTreeWidgetItem * pRootGroupItem, int DepID)
{
	...
	QString str23 = queryPicture.value(0).toString();
	//获取部门名称
	QString strDepName;
...
{
	setUserName(QString::fromLocal8Bit("奇牛科技"));
...
void CCMainWindow::onItemClicked(QTreeWidgetItem * item, int colume)
{
...
void CCMainWindow::onItemExpanded(QTreeWidgetItem * item)
{
	...
	if (!bIsChild)
	{
		//dyname_cast 将基类对象指针（或引用）装换到继承类指针
		RootContatltem * prootItem = dynamic_cast<RootContatltem*>(ui.treeWidget->itemWidget(item, 0));
		if (prootItem)
		{
...
void CCMainWindow::onItemDoubleClicked(QTreeWidgetItem * item, int colime)
{
	bool bIsChild = item->data(0, Qt::UserRole).toBool();
	if (bIsChild)
	{
