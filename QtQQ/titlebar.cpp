...
void TitleBar::initControl()
{
	...
	setWindowFlags(Qt::FramelessWindowHint);//设置窗口为无边框风格
...
//通过鼠标按下，鼠标移动，鼠标释放事件实现拖动标题栏达到移动窗口的效果
void TitleBar::mousePressEvent(QMouseEvent *event)
{
	if (m_buttonType == MIN_MAX_BUTTON)
	{
		...
		{
			m_isPressed = true;
			m_startMovePos = event->globalPos();//返回事件发生时鼠标在的全局位置
		}
	}
	else 
	{
