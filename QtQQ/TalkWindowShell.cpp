...
{
	ui.setupUi(this);
	setAttribute(Qt::WA_DeleteOnClose);//窗口关闭时，资源回收
	initControl();
...
void TalkWindowShell::addTalkWindow(TalkWindow * talkWindow, TalkWindowItem * talkWindowItem,const QString & uid /*, GroupType grouptype*/)
{
	...
	connect(m_emotionWindow, SIGNAL(signalEmotionWindowHide()), talkWindow, SLOT(onEmotionBtnClicked()));
...
void TalkWindowShell::initControl()
{
	...
	setWindowTitle(QString::fromLocal8Bit("懒鬼-聊天窗口"));
...
{
	//读取txt文件数据
	QString strFileText = "Resources/MainWindow/MsgHtml/msgtmpl.txt";
	QFile fileRead(strFileText);
	QString strFile;
	...
	//替换(external0,appendHeml0用作自己发消息使用
	...
	if (fileWrite.open(QIODevice::WriteOnly | QIODevice::Truncate))
	{
		...
		//更新追加recvHtml,脚本中有双引号无法直接进行赋值，采用对文件方式
...
//文件数据包格式:群聊标志 + 发信息员工QQ号 + 收信息员工号（群QQ号） + 信息类型 + 文件长度 + "bytes" + 文件名称 + "data_beign" + 文件内容
...
void TalkWindowShell::processPendingData()
{
	...
	while (m_udpReceiver->hasPendingDatagrams())
	{
		...
		//读取udp数据
		QByteArray btData;
		btData.resize(m_udpReceiver->pendingDatagramSize());//获取udp数据包的大小设置为btData的数据大小
		...
		QString strSendEmployeeID;//发送者的QQ号
		QString	strRecevieEmployeeID;//接收端的QQ号
		QString strMsg;//数据
