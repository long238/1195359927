...
void WindowManager::addNewTalkWindow(const QString & uid/*, GroupType groupType, const QString & strPeople*/)
{
	...
	QWidget* widget = findWindowName(uid);//查询是否创建了该uid的窗口
	if (!widget)
	{
		...
		{
			case COMPANY:
				talkwindow->setWindowName(QStringLiteral("奇牛科技——分享"));
				talkwindowItem->setMsgLabelContent(QStringLiteral("公司群"));

				break;
			case MARKETGROUP  :
				talkwindow->setWindowName(QStringLiteral("努力工作——分享"));
				talkwindowItem->setMsgLabelContent(QStringLiteral("市场群"));
				break;
			case DEVELOPMENTGROUP:
				talkwindow->setWindowName(QStringLiteral("编程——分享"));
				talkwindowItem->setMsgLabelContent(QStringLiteral("研发群"));
				break;
			case PERSONELGROUP:
				talkwindow->setWindowName(QStringLiteral("公心——分享"));
