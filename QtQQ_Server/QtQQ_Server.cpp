...
void QtQQ_Server::initComboBoxData()
{
	...
	int depCounts = queryDepModel.rowCount() - 1;//公司群 不算公司的分支部门
...
{
	for (quint16 prot = gUdpPort; prot < gUdpPort + 200; ++prot)
	{
		m_udpSender->writeDatagram(btData, btData.size(),QHostAddress::Broadcast, prot);
...
void QtQQ_Server::updateTableData(int depID, int employeeID)
{
	...
	headers << QStringLiteral("部门") << QStringLiteral("工号") << QStringLiteral("员工姓名") 
		<< QStringLiteral("员工签名") << QStringLiteral("员工状态") << QStringLiteral("员工头像") << QStringLiteral("在线状态");
...
void QtQQ_Server::on_logoutBtn_clicked()
{
	...
	else 
	{
		...
		QMessageBox::information(this, QString::fromLocal8Bit("提示"), QString::fromLocal8Bit("员工 %1 的企业QQ:%2 注销成功").arg(strName).arg(employeeID));
...
{
	//获取选中的头像路径
	m_pixPath = QFileDialog::getOpenFileName(this, QString::fromLocal8Bit("选择头像"), ".", "*.png;;*.jps");
	if (!m_pixPath.size())
	{
		return;
	}
...
void QtQQ_Server::on_addBtn_clicked()
{
	...
	//图片路径格式设置为 xxx\xxx\xxx.png 把"\"替换成"\"
	m_pixPath.replace("/", "\\\\");
